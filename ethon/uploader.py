...
async def ytdl(url):
    if 'HLS' in url:
        ...
        output = await bash(f'yt-dlp -f "bestvideo[height<=480]+bestaudio/best[height<=480]" --prefer-ffmpeg --no-warnings --yes-playlist {url}')
        return output 
  #  elif 'youtube' in url:
      #  await bash(f'yt-dlp -f "bestvideo[height<=480]+bestaudio/best[height<=480]" --prefer-ffmpeg --no-warnings {url}')
    else:
         await bash(f'yt-dlp -f "best[height<=480]" --prefer-ffmpeg --no-warnings {url}')
   #     await bash(f'yt-dlp -f "bestvideo[height<=480]+bestaudio/best[height<=480]" --prefer-ffmpeg --no-warnings {url}')
    o, e = await bash(f'yt-dlp -f "best[height<=480]" --get-filename {url}')
    with yt_dlp.YoutubeDL({'format': 'best[height<=480]'}) as ydl:
        info_dict = ydl.extract_info(url, download=False)
        video_title = info_dict.get('title', None) 
        video_ext = info_dict.get('ext', None) 
        file = f"{video_title}.{video_ext}"
